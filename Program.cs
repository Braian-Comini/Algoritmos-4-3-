...
class ConsoleApp2
{
    static void Main(String[] archivo)
    {
        ...
        for (int i = 0; i < escuela.GetLength(0); i++)
        {
            for (int f = 0; f < escuela.GetLength(1); f++)
            {
                ...
                while (true)
                {
                    if (letra == "S")
                    {
                        tieneInternet[i, 2] = true;
                        break;
                    }
                    else if (letra == "N")
                    {
                        tieneInternet[i, f] = false;
                        break;
                    }
                    else
                    {
        ...
        for(int i = 0; i < tieneInternet.GetLength(0); i++)
        {
            for(int f = 0; f < tieneInternet.GetLength(1); f++)
            {
                if (tieneInternet[i,f] == true)
                {
                    
                }
                Console.WriteLine($"¿Cuántos laboratorios en total tienen internet? {tieneInternet[i,f]}");
            }
        }
